...
static unsigned mapped_file_hash (const struct hash_elem *p_, void *aux UNUSED);
static bool mapped_file_less (const struct hash_elem *a_, const struct hash_elem *b_,
                      void *aux UNUSED);
...
void mapped_file_destory(struct hash_elem *e, void *aux UNUSED)
	{
		struct mapped_file *mapped_file = hash_entry(e, struct mapped_file, hash_elem);
		...
		while (size > 0)
		{
			...
			file_write_at (spt_entry->file, spt_entry->uaddr, spt_entry->file_page_size,
...
{
  const struct mapped_file *mapped_file_entry = hash_entry (p_, struct mapped_file, 
													hash_elem);
